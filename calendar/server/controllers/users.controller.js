...
class UsersController {
  ...
  static async addUser(req, res) {
    ...
    const insertId = await UsersService.addUser([String(name),user_level,String(user_name),String(password),String(email)]);
    if (insertId === 0)
  ...
  static async updateUser(req, res) {
    let id = parseInt(req.params.id);
    let { name, user_level, user_name, password, email} = req.body;
    const affectedRows = await CoursesService.updateCourse([String(name),user_level,String(user_name),String(password),String(email), id]);
    if (affectedRows === 0)
